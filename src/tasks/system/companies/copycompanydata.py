...
from ....db.models.functions import get_app_tables_choices
...
class CopyCompanyData(TaskRunner):
    def __init__(self, task_id, user_id, settings):
        super(CopyCompanyData, self).__init__(task_id, user_id, settings)
    ...
    def run(self):
        ...
        from_company = (session.query(sqa.get_model("Companies")).
                        get(self.params["fromcompany"]))
        to_company = (session.query(sqa.get_model("Companies")).
                        get(self.params["tocompany"]))

        self.context["data_row"] = [f"From Company: {from_company.Name}",
                                    f"To Company: {to_company.Name}",
                                    ""]
        ...
        self.context["data_row"].append("LEAF TABLES:")
        for leaf in leaves:
            ...
            total_tables += 1
            total_records += total
            self.context["data_row"].append(f"Table: {leaf}")
            self.context["data_row"].append(f"Total Records: {total}")
            self.context["data_row"].append(f"Created: {created}")
            self.context["data_row"].append("")

        self.context["data_row"].append("")
        self.context["data_row"].append("BRANCH TABLES:")
        for branch in branches:
            ...
            total_tables += 1
            total_records += total
            self.context["data_row"].append(f"Table: {branch}")
            self.context["data_row"].append(f"Total Records: {total}")
            self.context["data_row"].append(f"Created: {created}")
            self.context["data_row"].append("")

        self.context["data_row"].append("")
        self.context["data_row"].append("ROOT TABLES:")
        for root in ROOT_TABLES:
            ...
            total_tables += 1
            total_records += total
            self.context["data_row"].append(f"Table: {root}")
            self.context["data_row"].append(f"Total Records: {total}")
            self.context["data_row"].append(f"Created: {created}")
            self.context["data_row"].append("")

        self.context["data_row"].append("")
        self.context["data_row"].append(f"Total Tables: {total_tables}")
        self.context["data_row"].append(f"Total Records: {total_records}")
        self.context["data_row"].append(f"Total Records Created: {total_created}")
    ...
    def get_notify_message(self, spool_filename=None, attach_filename=None):
        return super(CopyCompanyData, self).get_notify_message(self.output_files[0], 'ImportTables.pdf')
