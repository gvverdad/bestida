...
class PurgeJob(TaskRunner):
    def __init__(self, task_id, user_id, settings):
        super(PurgeJob, self).__init__(task_id, user_id, settings)

    def run(self):
        self.context["title"] = "Purge Jobs : " + self.task.Title
        ...
        records = self.db_session.query(sqa.get_model("Tasks")).with_for_update(). \
            filter(sqa.where("Tasks.Status = 'Done' and Tasks.NextRunDateTime < {:%Y-%m-%d %H:%M:%S}".format(cut_off_date))).\
            all()
    ...
    def get_notify_message(self, spool_filename=None, attach_filename=None):
        return super(PurgeJob, self).get_notify_message(self.output_files[0], 'PurgeJobs.pdf')
