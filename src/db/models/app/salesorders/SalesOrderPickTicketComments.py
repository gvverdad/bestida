...
class SalesOrderPickTicketComment(Model):
    __versioned__ = {}
    __tablename__ = "SalesOrderPickTicketComments"
    __table_args__ = dict(info=dict(label="Sales Order PickTicket Comments",
                                    desc="Comments"))
    ...
    Type = relationship("SalesOrderCommentType",
                        primaryjoin="SalesOrderCommentType.Id==SalesOrderPickTicketComment.Type_Id")
    ...
    CreateOpId = relationship("User", primaryjoin="User.Id==SalesOrderPickTicketComment.CreateOpId_Id")
    CreateTimeStamp = Column(DateTime, default=datetime.utcnow,
                             info=dict(label="Created Timestamp", modifiable=False))
    # Many2One
    ModifiedOpId_Id = Column(Integer, ForeignKey("Users.Id"),
                             onupdate=get_current_uid, info=dict(label="Modified By", modifiable=False))
    ModifiedOpId = relationship("User", primaryjoin="User.Id==SalesOrderPickTicketComment.ModifiedOpId_Id")
