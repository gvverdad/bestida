from sqlalchemy import Column, ForeignKey, Integer

from ...address.Addresses import Address


class SalesOrderPickTicketAddress(Address):
    __versioned__ = {}
    __tablename__ = "SalesOrderPickTicketAddresses"
    __table_args__ = dict(info=dict(label="Sales Order PickTicket Addresses",
                                    desc="Addresses"))
        
    Id = Column(Integer, ForeignKey("Addresses.Id"), primary_key=True,
                nullable=False,
                info=dict(hidden=True))

    __mapper_args__ = {
        "polymorphic_identity": "SalesOrderPickTicket"
    }    
                                                  
    # ManyToOne side of SalesOrderPickTicket
    ItemSalesOrderPickTicket_Id = Column(Integer, ForeignKey("SalesOrderPickTickets.Id"),
                                 info=dict(hidden=True,
                                           exceptSchemaFields=["Addresses",
                                                               "CreateTimeStamp", "CreateOpId_Id", "CreateOpId",
                                                               "ModifiedTimeStamp", "ModifiedOpId_Id", "ModifiedOpId",
                                                               "versions"],
                                           ))
