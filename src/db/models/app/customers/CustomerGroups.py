...
class CustomerGroup(Model):
    ...
    __table_args__ = dict(info=dict(label="Customer Group",
    ...
    Company = relationship("Company", primaryjoin="Company.Id==CustomerGroup.Company_Id")                                      
    ...
    CreateOpId = relationship("User", primaryjoin="User.Id==CustomerGroup.CreateOpId_Id")                                  
    CreateTimeStamp = Column(DateTime, default=datetime.utcnow,
                             info=dict(label="Created Timestamp", modifiable=False))
    # Many2One    
    ModifiedOpId_Id = Column(Integer, ForeignKey("Users.Id"), 
                             onupdate=get_current_uid, info=dict(label="Modified By", modifiable=False))
    ModifiedOpId = relationship("User", primaryjoin="User.Id==CustomerGroup.ModifiedOpId_Id")
    ...
    def validate_delete(self, user_rowid, company_rowid, locale, timezone, data):
        ...
        if not is_ok:
            message = f"Cannot Delete. {tables} linked to this Customer Group"

        return is_ok, message


Index("CustomerGroup_Index1", CustomerGroup.Company_Id, CustomerGroup.Group, unique=True)
