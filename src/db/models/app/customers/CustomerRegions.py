...
class CustomerRegion(Model):
    __versioned__ = {}
    __tablename__ = "CustomerRegions"
    __table_args__ = dict(info=dict(label="Customer Region",
    ...
    Company = relationship("Company", primaryjoin="Company.Id==CustomerRegion.Company_Id")                                      
    ...
    Region = Column(String(32), nullable=False, info=dict(label="Region",
                                                          ...
                                                          selectKey="Region",
                                                          selectColumn="Region",
                                                          selectFormat=["Region", "Description"],
                                                          selectTable="CustomerRegions",
                                                          ))
    ...
    CreateOpId = relationship("User", primaryjoin="User.Id==CustomerRegion.CreateOpId_Id")                                  
    CreateTimeStamp = Column(DateTime, default=datetime.utcnow,
                             info=dict(label="Created Timestamp", modifiable=False))
    # Many2One    
    ModifiedOpId_Id = Column(Integer, ForeignKey("Users.Id"), 
                             onupdate=get_current_uid, info=dict(label="Modified By", modifiable=False))
    ModifiedOpId = relationship("User", primaryjoin="User.Id==CustomerRegion.ModifiedOpId_Id")
    ...
    def validate_delete(self, user_rowid, company_rowid, locale, timezone, data):
        ...
        if not is_ok:
            message = f"Cannot Delete. {tables} linked to this Customer Region"

        return is_ok, message


Index("CustomerRegion_Index1", CustomerRegion.Company_Id,
      CustomerRegion.Region, unique=True)
