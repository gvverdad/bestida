...
class CustomerType(Model):
    ...
    __table_args__ = dict(info=dict(label="Customer Type",
    ...
    Company = relationship("Company", primaryjoin="Company.Id==CustomerType.Company_Id")                                      
    ...
    CreateOpId = relationship("User", primaryjoin="User.Id==CustomerType.CreateOpId_Id")                                  
    CreateTimeStamp = Column(DateTime, default=datetime.utcnow,
                             info=dict(label="Created Timestamp", modifiable=False))
    # Many2One    
    ModifiedOpId_Id = Column(Integer, ForeignKey("Users.Id"), 
                             onupdate=get_current_uid, info=dict(label="Modified By", modifiable=False))
    ModifiedOpId = relationship("User", primaryjoin="User.Id==CustomerType.ModifiedOpId_Id")
    ...
    def validate_delete(self, user_rowid, company_rowid, locale, timezone, data):
        ...
        if not is_ok:
            message = f"Cannot Delete. {tables} linked to this Customer Type"

        return is_ok, message


Index("CustomerType_Index1", CustomerType.Company_Id, CustomerType.Type, unique=True)
