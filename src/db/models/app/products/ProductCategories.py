...
class ProductCategory(Model):
    ...
    __table_args__ = dict(info=dict(label="Product Categories",
    ...
    Company = relationship("Company", primaryjoin="Company.Id==ProductCategory.Company_Id")
        
    Category = Column(String(32), nullable=False, info=dict(label="Category",
                                                            case="uppercase",
                                                            selectId="Id",
                                                            selectKey="Category",
                                                            selectColumn="Category",
                                                            selectFormat=["Category", "Description"],
                                                            selectTable="ProductCategories",
                                                            ))
    ...
    CreateOpId = relationship("User", primaryjoin="User.Id==ProductCategory.CreateOpId_Id")
    CreateTimeStamp = Column(DateTime, default=datetime.utcnow,
                             info=dict(label="Created Timestamp", modifiable=False))
    # Many2One    
    ModifiedOpId_Id = Column(Integer, ForeignKey("Users.Id"), 
                             onupdate=get_current_uid, info=dict(label="Modified By", modifiable=False))
    ModifiedOpId = relationship("User", primaryjoin="User.Id==ProductCategory.ModifiedOpId_Id")
    ...
    ContractPrices = relationship("ContractPriceDiscount", uselist=False,
                               back_populates="Category",
                               primaryjoin="ContractPriceDiscount.Category_Id==ProductCategory.Id",
                               info=dict(hidden=True, dump=False))
    ...
    def validate_delete(self, user_rowid, company_rowid, locale, timezone, data):
        ...
        if not is_ok:
            message = f"Cannot Delete. {tables} linked to this Product Category"

        return is_ok, message


Index("ProductCategory_Index1", ProductCategory.Company_Id, ProductCategory.Category, unique=True)
