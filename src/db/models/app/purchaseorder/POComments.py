...
class POComment(Model):
    __versioned__ = {}
    __tablename__ = "POComments"
    __table_args__ = dict(info=dict(label="PO Comments", desc="Comments"))
    ...
    # Many2One
    Type_Id = Column(Integer, ForeignKey("POCommentTypes.Id"),
                     nullable=False,
                     info=dict(label="Type", selectId="Id",
                               selectKey="Description", depth=1))
    Type = relationship("POCommentType",
                        primaryjoin="POCommentType.Id==POComment.Type_Id")
    ...
    CreateOpId = relationship("User", primaryjoin="User.Id==POComment.CreateOpId_Id")
    CreateTimeStamp = Column(DateTime, default=datetime.utcnow,
                             info=dict(label="Created Timestamp", modifiable=False))
    # Many2One
    ModifiedOpId_Id = Column(Integer, ForeignKey("Users.Id"),
                             onupdate=get_current_uid, info=dict(label="Modified By", modifiable=False))
    ModifiedOpId = relationship("User", primaryjoin="User.Id==POComment.ModifiedOpId_Id")
