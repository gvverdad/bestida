...
class SupplierType(Model):
    __versioned__ = {}
    __tablename__ = "SupplierTypes"
    __table_args__ = dict(info=dict(label="Supplier Type",
    ...
    Company = relationship("Company", primaryjoin="Company.Id==SupplierType.Company_Id")                                      
    ...
    CreateOpId = relationship("User", primaryjoin="User.Id==SupplierType.CreateOpId_Id")                                  
    CreateTimeStamp = Column(DateTime, default=datetime.utcnow,
                             info=dict(label="Created Timestamp", modifiable=False))
    # Many2One    
    ModifiedOpId_Id = Column(Integer, ForeignKey("Users.Id"), 
                             onupdate=get_current_uid, info=dict(label="Modified By", modifiable=False))
    ModifiedOpId = relationship("User", primaryjoin="User.Id==SupplierType.ModifiedOpId_Id")
    ...
    def validate_delete(self, user_rowid, company_rowid, locale, timezone, data):
        ...
        if not is_ok:
            message = f"Cannot Delete. {tables} linked to this Supplier Type"

        return is_ok, message


Index("SupplierType_Index1", SupplierType.Company_Id, SupplierType.Type, unique=True)
