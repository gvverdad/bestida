...
@router.get("/purgespoolgridschema", response_model=GridSchema, tags=["pages"])
def get_purgespool_grid_schema(current_user_id: DBUser = Depends(get_current_user)):
...
@router.post("/purgespoolrulesets", response_model=ListResult, tags=["pages"])
def get_purgespool_rulesets(params: ListGridParams,
                            current_user_id: DBUser = Depends(get_current_user)):
    user = db.session.query(sqa.get_model("Users")).get(current_user_id)

    program = (db.session.query(sqa.get_model("Programs")).
               filter(sqa.where("Program = 'PurgeSpool' and Type = 'Script'")).first())
    if program is None:
        raise HTTPException(status_code=500,
                            detail=f"Invalid Program: PurgeSpool/Script")
...
@router.get("/purgespool", response_class=HTMLResponse, tags=["pages"])
def purgespool_page(request: Request,
                    current_user_id: DBUser = Depends(get_current_user)):
    ...
    return templates.TemplateResponse("pages/purges/purgespools.html", context)
