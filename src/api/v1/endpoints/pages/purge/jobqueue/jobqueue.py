...
@router.get("/purgejobsgridschema", response_model=GridSchema, tags=["pages"])
def get_purgejob_grid_schema(current_user_id: DBUser = Depends(get_current_user)):
...
@router.post("/purgejobsrulesets", response_model=ListResult, tags=["pages"])
def get_purgejob_rulesets(params: ListGridParams,
                          current_user_id: DBUser = Depends(get_current_user)):

    user = db.session.query(sqa.get_model("Users")).get(current_user_id)
    program = (db.session.query(sqa.get_model("Programs")).
               filter(sqa.where("Program = 'PurgeJobs' and Type = 'Script'")).first())
    if program is None:
        raise HTTPException(status_code=500,
                            detail=f"Invalid Program: PurgeJobs/Script")
...
@router.get("/purgejobs", response_class=HTMLResponse, tags=["pages"])
def purgejob_page(request: Request,
                  current_user_id: DBUser = Depends(get_current_user)):
    ...
    return templates.TemplateResponse("pages/purges/purgejobs.html", context)
