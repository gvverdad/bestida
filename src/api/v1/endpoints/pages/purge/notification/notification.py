...
@router.get("/purgenotificationsgridschema", response_model=GridSchema, tags=["pages"])
def get_purgenotifications_grid_schema(current_user_id: DBUser = Depends(get_current_user)):
...
@router.post("/purgenotificationsrulesets", response_model=ListResult, tags=["pages"])
def get_purgenotifications_rulesets(params: ListGridParams,
                                    current_user_id: DBUser = Depends(get_current_user)):

    user = db.session.query(sqa.get_model("Users")).get(current_user_id)
    program = (db.session.query(sqa.get_model("Programs")).
               filter(sqa.where("Program = 'PurgeNotifications' and Type = 'Script'")).first())
    if program is None:
        raise HTTPException(status_code=500,
                            detail=f"Invalid Program: PurgeNotifications/Script")
...
@router.get("/purgenotifications", response_class=HTMLResponse, tags=["pages"])
def purgenotifications_page(request: Request,
                            current_user_id: DBUser = Depends(get_current_user)):
    ...
    return templates.TemplateResponse("pages/purges/purgenotifications.html", context)
